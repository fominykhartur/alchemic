...
const GRAND_TEMPLATES = [
  ...
  (a) => `...${a} — лишь один из многих голосов в этом хоре; нужен весь хор целиком`,
  () => `...такое не делается с наскока — здесь нужен весь путь, пройденный с самого начала`,
  (a) => `...вспомнил про ${a} и понял: одного прошлого достижения мало, нужно больше`,
  () => `...в глубокой ночи привиделось: несколько моих находок сходятся в одной точке`,
  (a, cat) => `...${a} тянет за собой нити из ${cat} и ещё откуда-то — слишком много, чтобы понять сразу`,
  () => `...это Великое Делание в миниатюре: нужно собрать плоды многих трудов воедино`,
  () => `...чую сложную вязь — не пара ингредиентов, а целое созвездие уже открытого`,
];
...
// Сигнатура приведена к (a, cat) — раньше был (cat), а вызывается как t(name, cat),
// из-за чего в текст подставлялось название ингредиента вместо категории.
const ORACLE_GRAND_TEMPLATES = [
  (a, cat) => `...зеркало показывает не единственный путь, а перекрёсток многих; один ведёт из мира ${cat}`,
  (a, cat) => `...в глубине стекла вижу россыпь прежних находок — ${a} среди них, но не одна`,
  (a, cat) => `...отражение слишком сложное для одного взгляда: там сплетены и ${a}, и мир ${cat}`,
  (a, cat) => `...зеркало молчит о частностях, но ясно одно: путь лежит через уже пройденное и через ${cat}`,
  (a, cat) => `...в стекле мерцает созвездие свершений; одна из его точек — мир ${cat}`,
  (a, cat) => `...зеркало показывает ${a} в окружении многих спутников, и один из миров — ${cat}`,
];
...
const PROPHECY_GRAND_TEMPLATES = [
  ...
  (a, cat) => `...Хрономант видит не миг, а целую эпоху: ${a} лишь один из её знаков, другой — из ${cat}`,
  (a, cat) => `...будущее уже свершилось однажды: ${a} и весь пройденный путь сходятся в мире ${cat}`,
  (a, cat) => `...пророчество не о новом, а о собранном воедино: ${a} и отголоски из ${cat}`,
  (a, cat) => `...песочные часы показывают многое сразу: ${a} среди прочего, а дальше — ${cat}`,
  (a, cat) => `...Хрономант молчит о деталях, но твёрдо знает: путь через ${a} ведёт в ${cat}`,
];
